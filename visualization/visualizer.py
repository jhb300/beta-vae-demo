...
class Visualizer:
    ...
    def visualize(
        self, discrete_latent_parameters: list, continous_latent_parameters: list
    ) -> np.ndarray:
        "Visualize using the self model and a set of parameters, i.e. decoder input."

        assert (
            self.model != None
        ), "No model has been defined yet to run the visualizations on."

        concatenated_parameters = torch.tensor(
            [*continous_latent_parameters, *discrete_latent_parameters],
            dtype=torch.float,
        )
        with torch.no_grad():
